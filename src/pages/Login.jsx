...
export default function Login() {
  ...
  const portalOptions=[
    ...
    },
    {
      ...
      description:"Access your dashboard,view payslips,request leave, and manage attendance",
      icon:UserIcon
    }
  ]
  return (
...
<div className='space-y-4'>
  {portalOptions.map((portal)=> {
    return <Link key={portal.to} to={portal.to}
    className="group block bg-slate-50 border border-slate-200 rounded-lg p-5 sm:p-6
...
<div className='relative z-10 flex items-center justify-between gap-4 sm:gap-5'>
  ...
  <ArrowRightIcon className="w-4 h-4 text-slate-400 group-hover:text-indigo-600 group-hover:translate-x-1 transition-all duration-300"/>
        ...
        <div className='mt-12 text-center md:text-left text-sm text-slate-400'>
          <p>© {new Date().getFullYear()} Sandipan Das. All rights reserved.</p>
        </div>
      </div>
      </div>
    </div>
